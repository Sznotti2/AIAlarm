...
public class PermissionAdapter extends RecyclerView.Adapter<PermissionAdapter.ViewHolder> {
	...
	public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
		View listItem = LayoutInflater.from(parent.getContext()).inflate(R.layout.recyclerviewrow_reqperm, parent, false);
	...
	public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
		...
		switch (permission.permType()) {
			case ConstantsAndStatics.PERMISSION_LEVEL_ESSENTIAL -> {
				holder.permTypeTV.setText(context.getString(R.string.perm_type_essential));
			...
			case ConstantsAndStatics.PERMISSION_LEVEL_RECOMMENDED -> {
				...
				holder.permTypeTV.setTextColor(ConstantsAndStatics.isNightModeActive(context) ?	Color.parseColor("#c32b7a73") : Color.parseColor("#FFC869FF"));
			...
			case ConstantsAndStatics.PERMISSION_LEVEL_OPTIONAL -> {
				...
				holder.permTypeTV.setTextColor(
					ConstantsAndStatics.isNightModeActive(context) ? Color.parseColor("#FFB4FF90") : Color.parseColor("#FF282099"));
				holder.denyAccessBtn.setVisibility(View.VISIBLE);
			}
		}
